...
def tests_cleaningData():
    ...
    unredacted_file_test = unredactor.unredactor(redacted_file_train,'John')
    
    unredacted_file_testi = len(re.findall(u"[\u2588]+ ?[\u2588]+", unredacted_file_test[0]))
    
    
    assert unredacted_file_testi == 0 #8 features 
